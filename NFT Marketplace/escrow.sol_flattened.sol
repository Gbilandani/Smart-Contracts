...
// File: contracts/28 feb/Iescrow.sol
...
// File: contracts/28 feb/ERC20Contract.sol
...
contract ERC1155 is Context, ERC165, IERC1155, IERC1155MetadataURI {
    ...
    modifier checkpurchase(address owner, uint256 id, uint256 amount) {
        require(_balances[id][owner] >= amount, "Not enough tokens to purchase.");
        _;
    }
    ...
    function _purchase(
        ...
        uint256 id,
        uint256 amount
    )internal virtual checkpurchase(owner, id, amount) {
        _balances[id][owner] -= amount;
        _balances[id][to] += amount;
    }

    modifier checksell(address to, uint256 id, uint256 amount) {
        require(_balances[id][to] >= amount, "Not enough tokens to sell.");
        _;
    }
    ...
    function _sell(
        ...
        uint256 id,
        uint256 amount
    )internal virtual checksell(to, id, amount) {
        _balances[id][to] -= amount;
        _balances[id][owner] += amount;
    }
}

// File: contracts/28 feb/nftassignment.sol
...
// File: contracts/28 feb/escrow.sol
