...
contract MyHomeNft is ERC721, ERC721URIStorage, Ownable, ERC721Holder {
    ...
    event PropertyForSale(uint hid, address seller, string uri, string location, uint price, uint time );
    event PropertySold(uint hid, address buyer, address seller, string uri, string location, uint price, uint time);

    function Sale( string memory _image, string memory _location, uint256 _price) public  {
        _homeIdCounter.increment();
        uint256 homeId = _homeIdCounter.current();
        _safeMint(address(this), homeId);
        _setTokenURI(homeId, _image);
        hMap[homeId] = home(address(this), msg.sender, homeId, _image, _location, _price, true);
        emit PropertyForSale(homeId, msg.sender, hMap[homeId].image, hMap[homeId].location, hMap[homeId].price, block.timestamp);
    }
    ...
    modifier checkSeller(uint256 hid) {
        require(hMap[hid].seller != msg.sender, "You are the seller of the property, You can't buy it.");
        _;
    }

    function Buy(uint256 hid) public payable checkSale(hid) checkSeller(hid) checkPrice(hid) {
        ...
        emit PropertySold(hid, msg.sender, sellrr, hMap[hid].image, hMap[hid].location, msg.value, block.timestamp);
    }
