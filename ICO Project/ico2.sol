...
contract ICOGAURAV is Ownable { 
    ...
    function buyToken (uint256 _icoID) external payable checkICO(_icoID) checkStartTime(_icoID)  checkValue checkToken(_icoID) checkEndTime(_icoID)  {
        ...
        //     tokensAmount = tokenBalance(_icoID);
        // }
        ICOmap[_icoID].amount -= tokensAmount;
        emit ICOTokenBought(_icoID, msg.sender, tokensAmount, ICOmap[_icoID].pricePerToken, msg.value, tokenPrice, refundPrice, block.timestamp);
    }
